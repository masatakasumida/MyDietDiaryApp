...
//土日の色を変更
    ...
    func transitionToEditorView(with record: WeightRecord? = nil) {
        ...
        guard let editorViewController =
                storybord.instantiateInitialViewController() as? EditorViewController else { return }
...
//比較対象のDate型がの年月日が一致していた場合にtrueとなる
        let isEpualDate = dateList.contains(date.zeroclock)
        return isEpualDate ? 1 : 0
    }
